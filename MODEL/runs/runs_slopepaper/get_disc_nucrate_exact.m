...
CO23 = Coags_dR(Dp(dp2),in.sinkdist(1,:),in.sinkdist(2,:),in.temp);
...
ix_start2 = find(J2>0,1);
numvec = 1:length(J2);
ix_end2 = find((J2<0 & numvec'>5),1); % not at start
if isempty(ix_end2)
    ix_end2 = length(J2);
end
...
J2_mean = nanmedian(J2(ix_start2:ix_end2));
...
hold on
plot(dtim,J2./1e6,'k-')
plot(dtim,Fc2(2:end)./1e6,'k-.')
plot(dtim,Fg2(2:end)./1e6,'k--')
plot(dtim,dN2dt./1e6,'k:')
xlabel('Simulation time (s)')
ylabel('Rate (cm^{-3}s^{-1})')
legend('J2','FCoag','Fgrowth','dNdt')
...
ou.GR_data = GR_data;
ou.gamma2 = (1./(-1.7+1)).*((2e-9./Dp(nus)).^(-1.7)-1);
ou.factor2 = exp(-ou.gamma2.*Dp(nus).*1e9.*(delta_t_data.*CO23));
ou.J2_avg = J2_mean;
