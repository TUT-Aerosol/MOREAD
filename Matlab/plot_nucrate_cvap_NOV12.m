...
for i = 1:7,
   clear in out nu
    load(sprintf('T12_A_MOREAD_NOV12_test%02i',i));
...
plot(Cvap./(1e6.*vapsca),10.*madNdt1,'r*-')
hold on
plot(Cvap./(1e6.*vapsca),10.*madNdt1./nufact,'b*-')
...
for i = 1:7,
   clear in out nu
    load(sprintf('T12_B_MOREAD_NOV12_test%02i',i));
   nu= get_disc_nucrate(in,out);
   Cvap_co(i) = in.cvap_0;
...
hold on
plot(Cvap_co./(1e6.*vapsca),10*madNdt1_co,'ro-')
plot(Cvap_co./(1e6.*vapsca),10*madNdt1_co./nufact_co,'bo-')
