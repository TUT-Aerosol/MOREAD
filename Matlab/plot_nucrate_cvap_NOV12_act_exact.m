...
for i = 1:7,
   clear in out nu
    load(sprintf('T12_A_MOREAD_NOV12_test_act%02i',i));
   nu= get_disc_nucrate_exact(in,out);
   Cvap(i) = in.cvap_0;
   madNdt1(i)   = nu.ma_J1;
   medNdt1(i)   = nu.me_J1;
   mddNdt1(i)   = nu.md_J1;
...
plot(Cvap./(1e6.*vapsca),madNdt1,'r*-')
hold on
plot(Cvap./(1e6.*vapsca),madNdt1./nufact,'b*-')
plot(Cvap./(1e6.*vapsca),kineJ./10,'k*-')
...
for i = 1:7,
   clear in out nu
    load(sprintf('T12_B_MOREAD_NOV12_test_act%02i',i));
   nu= get_disc_nucrate_exact(in,out);
   Cvap_co(i) = in.cvap_0;
   madNdt1_co(i)   = nu.ma_J1;
   medNdt1_co(i)   = nu.me_J1;
   mddNdt1_co(i)   = nu.md_J1;
   nufact_co(i)    = nu.factor2_CS10;
   kineJ_co(i)     = nu.actiJ;
...
hold on
plot(Cvap_co./(1e6.*vapsca),madNdt1_co,'ro-')
plot(Cvap_co./(1e6.*vapsca),madNdt1_co./nufact_co,'bo-')
